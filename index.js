...
function HE_ST_Platform(log, config, api) {
    ...
    this.doVersionCheck();
    he_st_api.init(this.hubconnect_key, this);
    this.receiver = require('./lib/receiver_hubconnect.js').receiver;
    ...
    reloadData: function(callback) {
        ...
        he_st_api.getDevicesSummary().then(function(myList) {
            ...
            if (error.hasOwnProperty('statusCode'))
            {
                if (error.statusCode === 404)
                {
                    that.log.error('Hubitat tells me that the HubConnect instance you have configured is not available (code 404).');
                }
                else if (error.statusCode === 401)
                {
                ...
                {
                    that.log.error('Looks like your HubConnect instance is disabled. Got code 500');
                }
                else
                {
