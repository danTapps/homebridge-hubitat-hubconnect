...
var receiver_hubconnect = {
    start: function(platform) {
	    ...
	    retVal['installedVersion']['appVersion'] = version;
            retVal['prefs'] = {};
      ...
      status.text('Error');
        ...
        platform.api.connect("http://" + platform.local_ip + ":" + platform.local_port,
            ...
            if (data) {
                if (data.status === 'success')
                    ...
                    platform.log('Got an error back from HubConnect')
                    platform.log(data.message);
                    platform.log('Going to exit here. Please check the HubConnect logs');
                    setTimeout(function() {
                        process.exit(1);
                    }, 3000);
                }
        ...
        function connect(platform) {
            ...
            ws.onmessage = function(e) {
                ...
                platform.log.debug('Received new ws message: ', e.data);
                try {
                ...
                if (jsonData['source'] === 'DEVICE') {
                    ...
                    switch (jsonData['name']) {
                        case 'hsmStatus':
                            platform.log('Received HSM Status');
                            newChange.push( {
                                device: 'hsm' + platform.config['name'],
                                displayName: 'Alarm System ' + platform.config['name'],
                                attribute:  'alarmSystemStatus',
